document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyServerResult(data) {
    Object.keys(map).forEach((k) => {
      ...
      if (msg) {
        ...
        if (err) {
            err.textContent = msg;
            err.classList.add("d-block"); // <-- force l'affichage
        }
      } else {
  ...
  async function callValidate() {
    ...
    if (!res.ok) throw new Error("Erreur serveur. 1");
    return res.json();
  }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (data.ok) {
        setStatus("success", "Validation OK ✅ Connexion...");
        form.submit();
      } else {
        setStatus("danger", "Veuillez corriger les erreurs.");
      }
    } catch (err) {
      setStatus("warning", err.message || "Erreur inconnue.");
    }
  });
